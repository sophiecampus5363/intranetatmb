...
if (!is_admin()) {
    ...
    wp_enqueue_style(
        'woffice-extension-' . $ext_instance->get_name() . '-calendar-datetimepicker',
        woffice_get_extension_uri('event', 'static/css/jquery.datetimepicker.css'),
        array(),
    ...
    wp_enqueue_script(
        'woffice-events-moments',
        woffice_get_extension_uri('event', 'static/js/moment.min.js'),
        array('jquery', 'woffice-theme-script'),
        '1.0',
        true
    );
    ...
    wp_enqueue_script(
        'woffice-events-main-datetime',
        woffice_get_extension_uri('event', 'static/js/jquery.datetimepicker.js'),
        array('jquery', 'woffice-theme-script'),
        '1.0',
        true
    );
    ...
    $events_config = apply_filters('woffice_events_frontend_config', array(
	    ...
	    'starting_day'    => ucfirst(fw_get_db_ext_settings_option($ext_instance->get_name(), 'woffice_calendar_starting_day')),
	    'month'           => date('m'),
	    'year'            => date('Y'),
	    'day'             => date('d'),
