			...
			jQuery(function () {
                ...
                foreach($js_array as $location) {
					if(is_object(get_userdata($location->user_id))){
                        ...
                        $name_to_display = woffice_get_name_to_display($location->user_id);
	                    if(is_array($buddy_excluded_directory_ready) && count(array_intersect($buddy_excluded_directory_ready, $user->roles)) == 0) {
	                        $avatar_url = fw()->extensions->get( 'woffice-map' )->getAvatarUrl(get_avatar($location->user_id,100));
...
//                            $info_box_content = '<div class=\"user-map-box\"><a href=\"'.$profile_url.'\">'.$echo_avatar.'<h3>'.esc_html($name_to_display).'</h3><p>'.esc_html($location->name).'</p></a></div>';
