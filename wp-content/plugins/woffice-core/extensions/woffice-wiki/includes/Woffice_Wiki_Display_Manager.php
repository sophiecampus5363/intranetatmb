...
if( ! class_exists( 'Woffice_Wiki_Display_Manager' ) ) {
	...
	class Woffice_Wiki_Display_Manager {
		...
		public function __construct( $parent = 0 ) {
			...
			$this->excludedCateogires = ( ! empty( $wiki_excluded_categories ) ) ? $wiki_excluded_categories : array();
			 ...
			 * @param array
			 */
			$args = apply_filters('woffice_wiki_items_query_args', array(
				'post_type' => 'wiki',
				'showposts' => '-1',
				'orderby' => 'post_title',
				'order' => 'ASC',
		...
		protected function getCategoriesArgs( $child_of, $include) {
			...
			return apply_filters('woffice_wiki_categories_query_args', array(
				...
				'child_of'  => $child_of,
				'hide_empty' => true,
				'exclude'  => $this->excludedCateogires,
			));
