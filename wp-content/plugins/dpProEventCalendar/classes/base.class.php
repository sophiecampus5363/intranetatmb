...
class DpProEventCalendar {
	...
	function getCalendarData() {
		...
		if(isset($this->calendar_obj->link) && $this->calendar_obj->link != "") {
			if(substr($this->calendar_obj->link, 0, 4) != "http") {
		...
		switch($get) {
			...
			case 'link':
				if($event_data->link != "") {
					...
					if(substr($event_data->link, 0, 4) != "http") {
			...
			case 'book_event':
				...
				$return .= $this->getBookingBookedLabel($post_id, $event_data->date);
	...
	function monthlyCalendarLayout($compact = false) 
	{
		...
		*/
		//die();
			...
			if($this->calendar_obj->hide_old_dates && date("Y-m") == $year."-".$month_number) {
	...
	function singleEventLayout($result, $search = false, $selected_date = null, $show_end_date = true, $type = '', $force = false) {
		...
		foreach($result as $event) {
			...
			if($this->type != 'compact') {
				...
				$html .= $this->getBookingBookedLabel($event->id, ($selected_date != null ? $selected_date : date('Y-m-d', strtotime($event->date))));
	...
	function output( $print = false ) 
	{
		...
		if($this->type == 'calendar') {
			...
			$html .= "<div class='dp_pec_clear'></div>
				...
				if($this->calendar_obj->hide_old_dates && $this->datesObj->currentMonth == date('m') && $this->datesObj->currentYear == date('Y')) {
