...
function dpProEventCalendar_getDate() {
   ...
   //die(__FILE__.'/../classes/base.class.php');
	...
	$dpProEventCalendar = new DpProEventCalendar( $is_admin, $calendar, $timestamp, null, '', $category, $event_id, $author, '', '', '', '', 0, array('location' => $location, 'include_all_events' => $include_all_events ));
...
function dpProEventCalendar_getDaily() {
	...
	$dpProEventCalendar = new DpProEventCalendar( $is_admin, $calendar, $timestamp, null, '', $category, $event_id, $author, '', $columns, '', '', 0, array('location' => $location, 'include_all_events' => $include_all_events ));
...
function dpProEventCalendar_getEvents() {
	...
	$dpProEventCalendar_class = new DpProEventCalendar( false, $calendar, null, null, '', $category, $event_id, $author, '', '', '', '', 0, array('location' => $location, 'include_all_events' => $include_all_events ) );
...
function dpProEventCalendar_getEventsMonth() {
	...
	$dpProEventCalendar = new DpProEventCalendar( false, $calendar, null, null, '', $category, $event_id, $author, '', '', '', '', 0, array('location' => $location, 'include_all_events' => $include_all_events ) );
...
function dpProEventCalendar_getEventsMonthList() {
	...
	$dpProEventCalendar = new DpProEventCalendar( false, $calendar, null, null, '', $category, $event_id, $author, '', $columns, '', '', 0, array('location' => $location, 'include_all_events' => $include_all_events ) );
...
function dpProEventCalendar_contentFilter($content) {
	...
	if(is_array($GLOBALS) && null !== $GLOBALS['post']) {
		...
		if ($GLOBALS['post']->post_type == 'pec-events' && $wp_query->is_single && !post_password_required($GLOBALS['post']->ID)) {
			...
			$content = '<div class="dp_pec_event_page_nav">'.
...
function dpProEventCalendar_importFB($calendar_id, $event_url, $category_ics = '', $event_option = 2, $offset = '') {
	...
	foreach($event_list as $key) {
		...
		try {
		$response = $facebook->api('/'.$event_url, 'get', array('fields' => array('cover', 'description', 'end_time', 'name', 'place', 'start_time', 'ticket_uri')));
